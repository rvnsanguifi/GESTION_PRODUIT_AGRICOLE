package com.chef_dexp.Fournisseur;
...
public class Fournisseur extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<h1>Voici la liste des Fournisseurs </h1>");
		out.println("<form name=\"f\" action=\"Fournisseur\">");

		out.println("<fieldset style=\"width: 500px\">");
		out.println("<legend>Choisir un nom</legend>");
		out.println("<label>Fournisseurs</label> ");
		out.println("<select name=\"Fournisseur\"  >"); 
		out.println("	<option value='-1'>Aucun</option>");
		 
			resultat=connexion.getData("select CODEFOUR,nom from Fournisseur");
		...
		out.println("</select>"); 
		out.println("<div id='Fournisseur' >"); 
		out.println("<table border=1 width=100% bordercolor=black>");
		out.println("<br>");
		
			
			 resultat=connexion.getData("SELECT * FROM Fournisseur WHERE CODEFOUR="+request.getParameter("Fournisseur")+" ORDER BY nom");
			 for(int i=0;i<resultat.size();++i)
			 {
				 resultat_final=(Vector)resultat.elementAt(i);
				
				
					out.println("<tr><th>Code du fournisseur</th><td>"+resultat_final.elementAt(0)+"</td></tr>");
					out.println("<tr style=\"background-color:#DAFAC9;\"><th>Nom du fournisseur</th><td>"+resultat_final.elementAt(1)+"</td></tr>");
